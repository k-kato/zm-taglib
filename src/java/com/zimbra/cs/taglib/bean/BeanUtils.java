...
public class BeanUtils {
	...
	public static String displayVoiceDate(PageContext pc, Date msg) throws ServiceException, JspException {
		...
		String resource = "ZM_formatVoiceDate";;
		if (nowTime - msgTime < ONE_DAY && cal.get(Calendar.DAY_OF_WEEK) == msgCal.get(Calendar.DAY_OF_WEEK)) {
			resource = "ZM_formatVoiceDateToday";
		} else if ((nowTime - msgTime) < (2 * ONE_DAY) ) {
			cal.setTimeInMillis (nowTime - ONE_DAY);
			if (cal.get(Calendar.DAY_OF_WEEK) == msgCal.get(Calendar.DAY_OF_WEEK)) {
	...
	static {
		sPHONE_FIELDS.put(Contact.A_callbackPhone, "phoneLabelCallback");
		sPHONE_FIELDS.put(Contact.A_carPhone, "phoneLabelCar");
		sPHONE_FIELDS.put(Contact.A_assistantPhone, "phoneLabelAssistant");
		sPHONE_FIELDS.put(Contact.A_companyPhone, "phoneLabelCompany");
		...
		sPHONE_FIELDS.put(Contact.A_assistantPhone, "phoneLabelAssistant");
		sPHONE_FIELDS.put(Contact.A_companyPhone, "AB_FIELD_company");
	}
